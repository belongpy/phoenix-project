async for message in self.client.iter_messages(entity, offset_date=time_limit):
                ...
                if contracts or kols:
                    logger.info(f"Found contracts: {contracts}, KOLs: {kols}")
                
                # Create call entries
                for contract in contracts:    def extract_contracts(self, text: str) -> List[str]:
        ...
        patterns = [
            ...
            # Contract/CA mentions
            r'(?i)(?:contract|ca|address)[\s:]+([1-9A"""
Telegram Module - Phoenix Project (Updated)

Focused on SpyDefi analysis with simple flow:
...
class TelegramScraper:
    """Simplified Telegram scraper focused on SpyDefi performance analysis."""
        ...
        for pattern in patterns:
            ...
            for match in matches:
                ...
                if not addr.startswith('0x') and 32 <= len(addr) <= 44:
                    ...
                    if re.match(r'^[1-9A-HJ-NP-Za-km-z]+
...
# Legacy methods for backward compatibility
    ...
    async def export_spydefi_analysis(self, analysis: Dict[str, Any], output_file: str) -> None:
        """Legacy method - redirects to new export method."""
        await self.export_analysis_results(analysis, output_file), addr):
                        contracts.add(addr)
